...
try: import mediapipe as mp; _MP=hasattr(mp,"solutions")
except: mp=None; _MP=False
...
def generate_pool():
    ...
    for _ in range(700):
        ...
        pool.append({**m,"id":f"{m['id']}_{random.randint(0,99999)}","base_id":m["id"],
            "domain":"Motor","protocol":random.choice(["ABA-DTT","ESDM"]),
            "tablet_mode":"motor","tokens":2,"joy":"dance"})
    for _ in range(400): pool.append(_grid(COLORS,"color","Cognitive","TEACCH",3))
    for _ in range(350): pool.append(_grid(ANIMALS,"animal","Cognitive","TEACCH",4))
    for _ in range(300): pool.append(_grid(FRUITS,"fruit","Cognitive","TIE",4))
    for _ in range(250): pool.append(_grid(SHAPES,"shape","Cognitive","TEACCH",4))
    for _ in range(200): pool.append(_grid(EMOTIONS_ITEMS,"emotion","Social","ESDM",5))
    for _ in range(200): pool.append(_grid(FOODS,"food","Daily","TIE",4))
    for _ in range(200): pool.append(_grid(VEHICLES,"vehicle","Cognitive","TEACCH",4))
    for _ in range(200): pool.append(_grid(BODY,"body","Verbal","DTT",4))
    for _ in range(450):
        ...
        pool.append({"id":f"count_{n}_{random.randint(0,99999)}","base_id":f"count_{n}",
            ...
            "tablet_mode":"number","target_number":n,"verify":"finger_count","tokens":5,"joy":"celebrate",
    ...
    for _ in range(500):
        ...
        pool.append({"id":f"say_{w}_{random.randint(0,99999)}","base_id":f"say_{w}",
            ...
            "tokens":4,"joy":"dance","prompts":[f"Say {w}!","Press mic!","Loud and clear!"]})
    for _ in range(400):
        ph=random.choice(SOCIAL)
        pool.append({"id":f"phrase_{ph.replace(' ','_')}_{random.randint(0,99999)}",
            "base_id":f"phrase_{ph.replace(' ','_')}","domain":"Social","protocol":"ESDM",
            "name":f"💬 '{ph}'","instruction":f"Say: {ph}!","waiting":f"Say {ph}! 🎤",
            "success":f"Great! '{ph}'! ✅","fail":f"Try: {ph}!","tablet_mode":"social",
            "social_text":ph,"verify":"speech_keyword","keyword":ph,"tokens":6,"joy":"full_joy",
            "prompts":[f"Say {ph}!","You can do it!","Speak up!"]})
    random.shuffle(pool)
    log.info(f"✅ Generated {len(pool)} tasks")
    return pool
...
class CameraThread(QThread):
    ...
    def _init_mp(self):
        ...
        if _MP:
            ...
            for name,cls,kw in [
                ("Pose",mp.solutions.pose.Pose,{"min_detection_confidence":0.55,"model_complexity":1}),
                ("Hands",mp.solutions.hands.Hands,{"max_num_hands":2,"min_detection_confidence":0.60}),
                ("FaceMesh",mp.solutions.face_mesh.FaceMesh,{"max_num_faces":1,"min_detection_confidence":0.5,"refine_landmarks":True}),
            ]:
                try:
        ...
        try:
            self._cv_face=cv2.CascadeClassifier(cv2.data.haarcascades+"haarcascade_frontalface_default.xml")
            self._cv_eye=cv2.CascadeClassifier(cv2.data.haarcascades+"haarcascade_eye.xml")
            self._cv_hog=cv2.HOGDescriptor(); self._cv_hog.setSVMDetector(cv2.HOGDescriptor_getDefaultPeopleDetector())
            self._cv_bg=cv2.createBackgroundSubtractorMOG2(history=120,varThreshold=40,detectShadows=False)
            log.info("✅ OpenCV vision (Haar+HOG+MOG2)")
        except Exception as e:
            log.warning(f"OpenCV vision init: {e}"); self._cv_face=None; self._cv_hog=None; self._cv_bg=None
    ...
    def run(self):
        ...
        while self.running:
            ...
            elif self._cv_hog and fc%8==0:
                try:
                    small=cv2.resize(frame,(320,240))
                    rects,_=self._cv_hog.detectMultiScale(small,winStride=(8,8),padding=(4,4),scale=1.05)
                    if len(rects)>0:
                        ST["face_detected"]=True; ST["attention"]=min(100,ST["attention"]+1)
                        # estimate hand raised from body region top quarter
                        for (rx,ry,rw,rh) in rects[:1]:
                            cv2.rectangle(frame,(rx*2,ry*2),((rx+rw)*2,(ry+rh)*2),(0,255,100),2)
                        ST["hand_raised"]=ST.get("body_motion",0)>8
                        ST["hands_up"]=ST.get("body_motion",0)>18
                except: pass
            if self._face and _MP and fc%3==0:
                try:
                    rgb=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
            ...
            elif self._cv_face and fc%3==0:
                try:
                    h_,w_=frame.shape[:2]
                    gray_f=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
                    faces=self._cv_face.detectMultiScale(gray_f,1.1,5,minSize=(60,60))
                    if len(faces)>0:
                        ST["face_detected"]=True; ST["attention"]=min(100,ST["attention"]+2)
                        fx,fy,fw,fh=faces[0]
                        cv2.rectangle(frame,(fx,fy),(fx+fw,fy+fh),(0,255,200),2)
                        roi=gray_f[fy:fy+fh,fx:fx+fw]
                        eyes=self._cv_eye.detectMultiScale(roi,1.1,3)
                        # crude emotion: if eyes visible→neutral/happy based on mouth region brightness
                        mouth_y=fy+int(fh*0.65); mouth_roi=gray_f[mouth_y:fy+fh,fx:fx+fw]
                        mb=float(np.mean(mouth_roi)) if mouth_roi.size>0 else 128
                        ST["emotion"]="happy" if mb>140 else "neutral"
                        ST["emotion_pct"]={k:0 for k in ST.get("emotion_pct",{})}
                        ST["emotion_pct"][ST["emotion"]]=100
                        ST["head_tilt"]=(fx+fw/2)/w_-.5
                    else:
                        ST["face_detected"]=False; ST["attention"]=max(0,ST["attention"]-2)
                except: pass
            if self._hands and _MP and fc%2==0:
                try:
                    rgb=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
            ...
            elif self._cv_bg and fc%2==0:
                try:
                    h_,w_=frame.shape[:2]
                    hsv=cv2.cvtColor(frame,cv2.COLOR_BGR2HSV)
                    # skin mask
                    mask=cv2.inRange(hsv,np.array([0,30,60]),np.array([20,170,255]))
                    mask|=cv2.inRange(hsv,np.array([160,30,60]),np.array([180,170,255]))
                    mask=cv2.morphologyEx(mask,cv2.MORPH_OPEN,np.ones((5,5),np.uint8))
                    # background subtraction for motion blobs
                    fg=self._cv_bg.apply(frame)
                    combined=cv2.bitwise_and(mask,fg)
                    cnts,_=cv2.findContours(combined,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
                    hand_regions=[(cv2.boundingRect(c),cv2.contourArea(c)) for c in cnts if 800<cv2.contourArea(c)<40000]
                    hand_regions.sort(key=lambda x:-x[1])
                    if hand_regions:
                        (hx,hy,hw,hh),_=hand_regions[0]
                        cv2.rectangle(frame,(hx,hy),(hx+hw,hy+hh),(255,200,0),2)
                        # crude finger estimate from contour convexity
                        big=[c for c in cnts if cv2.contourArea(c)>800]
                        if big:
                            hull=cv2.convexHull(big[0],returnPoints=False)
                            try:
                                defects=cv2.convexityDefects(big[0],hull)
                                fc_n=0
                                if defects is not None:
                                    for i in range(defects.shape[0]):
                                        s,e,f,d=defects[i,0]
                                        if d/256>20: fc_n+=1
                                ST["finger_count"]=min(5,fc_n+1)
                            except: ST["finger_count"]=1
                        cv2.putText(frame,f"Skin:{len(hand_regions)}",(hx,hy-5),cv2.FONT_HERSHEY_SIMPLEX,.5,(255,200,0),1)
                    else:
                        ST["finger_count"]=0
                except: pass
  ...
  if(t==='overview'){
    ...
    <div class="card"><h3>😊 State</h3>
      <div style="display:flex;flex-wrap:wrap;gap:6px;font-size:.83em">
        ...
        <span>| Voice:<b> ${st.current_voice||'friendly'}</b></span>
