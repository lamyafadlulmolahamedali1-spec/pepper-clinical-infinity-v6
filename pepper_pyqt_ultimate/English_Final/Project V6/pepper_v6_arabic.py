...
"""
╔══════════════════════════════════════════════════════════════════╗
║  بيبر كلينيكال إنفينيتي V6 — النسخة العربية الكاملة            ║
║  Pepper Clinical Infinity V6 — Full Arabic Version              ║
║  للمراكز العربية لعلاج التوحد                                   ║
╚══════════════════════════════════════════════════════════════════╝
"""

import os, sys, warnings, ctypes, signal, logging, threading
import subprocess, socket, time, random, math, re, json, csv
import base64, wave, tempfile, secrets
...
from PyQt6.QtCore import (
    ...
    QMutex, QMutexLocker, QPropertyAnimation, QEasingCurve,
    QPoint, QRect, QSize,
)
...
print("\n" + "═"*58)
print("  بيبر كلينيكال إنفينيتي V6 — النسخة العربية الكاملة")
print("  للمراكز العربية لعلاج اضطراب طيف التوحد")
print("═"*58)
CHILD_NAME = input("\n👦 اسم الطفل: ").strip() or "الطفل"
CHILD_AGE  = input("   العمر (الافتراضي 6): ").strip() or "6"
SAFE_NAME  = re.sub(r"[^a-zA-Z0-9_\u0600-\u06FF]", "_", CHILD_NAME)
CSV_FILE   = f"{SAFE_NAME}_v6_نتائج.csv"

with open(CSV_FILE, "w", newline="", encoding="utf-8-sig") as f:
    csv.writer(f).writerow([
        "الوقت","الطفل","المهمة","المجال","البروتوكول",
        "النتيجة","المتتالي","النقاط","المشاعر","الانتباه","مستوى المحادثة","الزمن_ms"])

def log_csv(tid, dom, proto, result, con, sc, em, att, clvl, ms):
    try:
        with open(CSV_FILE, "a", newline="", encoding="utf-8-sig") as f:
            csv.writer(f).writerow([
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                CHILD_NAME, tid, dom, proto,
                "نجاح" if result else "فشل",
                con, sc, em, att, clvl, int(ms)])
    except: pass
...
# ══════════════════════════════════════════════════════════════════
# بيانات المهام العربية — Arabic Task Data
# ══════════════════════════════════════════════════════════════════
...
# الحيوانات
ANIMALS_AR = [
    {"id":"كلب",    "emoji":"🐶","label":"كلب"},
    {"id":"قطة",    "emoji":"🐱","label":"قطة"},
    {"id":"أسد",    "emoji":"🦁","label":"أسد"},
    {"id":"فيل",    "emoji":"🐘","label":"فيل"},
    {"id":"أرنب",   "emoji":"🐰","label":"أرنب"},
    {"id":"دب",     "emoji":"🐻","label":"دب"},
    {"id":"قرد",    "emoji":"🐵","label":"قرد"},
    {"id":"نمر",    "emoji":"🐯","label":"نمر"},
    {"id":"سمكة",   "emoji":"🐟","label":"سمكة"},
    {"id":"عصفور",  "emoji":"🐦","label":"عصفور"},
    {"id":"بقرة",   "emoji":"🐄","label":"بقرة"},
    {"id":"حصان",   "emoji":"🐎","label":"حصان"},
    {"id":"خروف",   "emoji":"🐑","label":"خروف"},
    {"id":"بطة",    "emoji":"🦆","label":"بطة"},
    {"id":"ضفدع",   "emoji":"🐸","label":"ضفدع"},
    {"id":"فراشة",  "emoji":"🦋","label":"فراشة"},
]
# الفواكه
FRUITS_AR = [
    {"id":"تفاح",    "emoji":"🍎","label":"تفاح"},
    {"id":"موز",     "emoji":"🍌","label":"موز"},
    {"id":"برتقال",  "emoji":"🍊","label":"برتقال"},
    {"id":"عنب",     "emoji":"🍇","label":"عنب"},
    {"id":"فراولة",  "emoji":"🍓","label":"فراولة"},
    {"id":"مانجو",   "emoji":"🥭","label":"مانجو"},
    {"id":"بطيخ",    "emoji":"🍉","label":"بطيخ"},
    {"id":"خوخ",     "emoji":"🍑","label":"خوخ"},
    {"id":"كمثرى",   "emoji":"🍐","label":"كمثرى"},
    {"id":"كرز",     "emoji":"🍒","label":"كرز"},
    {"id":"أناناس",  "emoji":"🍍","label":"أناناس"},
    {"id":"كيوي",    "emoji":"🥝","label":"كيوي"},
]
# الأشكال
SHAPES_AR = [
    {"id":"دائرة",   "emoji":"⭕","label":"دائرة"},
    {"id":"مربع",    "emoji":"⬛","label":"مربع"},
    {"id":"مثلث",    "emoji":"🔺","label":"مثلث"},
    {"id":"نجمة",    "emoji":"⭐","label":"نجمة"},
    {"id":"قلب",     "emoji":"❤️","label":"قلب"},
    {"id":"معين",    "emoji":"💎","label":"معين"},
    {"id":"مستطيل",  "emoji":"▬","label":"مستطيل"},
    {"id":"بيضاوي",  "emoji":"🥚","label":"بيضاوي"},
]
# المشاعر
EMOTIONS_AR = [
    {"id":"سعيد",  "emoji":"😊","label":"سعيد"},
    {"id":"حزين",  "emoji":"😢","label":"حزين"},
    {"id":"غاضب",  "emoji":"😠","label":"غاضب"},
    {"id":"خائف",  "emoji":"😨","label":"خائف"},
    {"id":"مندهش", "emoji":"😲","label":"مندهش"},
    {"id":"متحمس", "emoji":"😄","label":"متحمس"},
    {"id":"متعب",  "emoji":"😴","label":"متعب"},
    {"id":"بخير",  "emoji":"👍","label":"بخير"},
]
# الوجبات
FOODS_AR = [
    {"id":"أرز",   "emoji":"🍚","label":"أرز"},
    {"id":"خبز",   "emoji":"🍞","label":"خبز"},
    {"id":"بيض",   "emoji":"🥚","label":"بيض"},
    {"id":"حليب",  "emoji":"🥛","label":"حليب"},
    {"id":"تمر",   "emoji":"🌴","label":"تمر"},
    {"id":"عسل",   "emoji":"🍯","label":"عسل"},
    {"id":"جبن",   "emoji":"🧀","label":"جبن"},
    {"id":"حساء",  "emoji":"🍲","label":"حساء"},
]
# المركبات
VEHICLES_AR = [
    {"id":"سيارة",  "emoji":"🚗","label":"سيارة"},
    {"id":"حافلة",  "emoji":"🚌","label":"حافلة"},
    {"id":"قطار",   "emoji":"🚂","label":"قطار"},
    {"id":"طائرة",  "emoji":"✈️","label":"طائرة"},
    {"id":"قارب",   "emoji":"⛵","label":"قارب"},
    {"id":"دراجة",  "emoji":"🚲","label":"دراجة"},
    {"id":"شاحنة",  "emoji":"🚚","label":"شاحنة"},
    {"id":"إسعاف",  "emoji":"🚑","label":"إسعاف"},
]
# أجزاء الجسم
BODY_AR = [
    {"id":"رأس",  "emoji":"🗣️","label":"رأس"},
    {"id":"عين",  "emoji":"👁️","label":"عين"},
    {"id":"أذن",  "emoji":"👂","label":"أذن"},
    {"id":"أنف",  "emoji":"👃","label":"أنف"},
    {"id":"فم",   "emoji":"👄","label":"فم"},
    {"id":"يد",   "emoji":"✋","label":"يد"},
    {"id":"قدم",  "emoji":"🦶","label":"قدم"},
    {"id":"ذراع", "emoji":"💪","label":"ذراع"},
]

# التمارين الحركية بالعربية
MOTORS_AR = [
    {"id":"تصفيق","name":"👏 صفّق","verify":"clap",
     "instruction":"صَفِّق يديك!","waiting":"صَفِّق الآن! 👏",
     "success":"رائع! صَفَّقت! ✅","fail":"ضع يديك معاً!",
     "prompts":["صَفِّق!","اليدان معاً!","هكذا! 👏"]},
    {"id":"تلويح","name":"👋 لوِّح","verify":"wave",
     "instruction":"لوِّح بيدك!","waiting":"لوِّح الآن! 👋",
     "success":"ممتاز! ✅","fail":"حرِّك يدك يميناً ويساراً!",
     "prompts":["لوِّح!","مرحباً!","يميناً ويساراً!"]},
    {"id":"رفع_يد","name":"✋ ارفع يدك","verify":"raise_hand",
     "instruction":"ارفع يدك عالياً!","waiting":"اليد لأعلى! ✋",
     "success":"ممتاز جداً! ✅","fail":"ارفع ذراعك فوق رأسك!",
     "prompts":["ارفع!","أعلى!","للأعلى!"]},
    {"id":"لمس_الأنف","name":"👆 المس أنفك","verify":"touch_nose",
     "instruction":"المس أنفك!","waiting":"المس أنفك! 👆",
     "success":"أحسنت! ✅","fail":"أشر إلى أنفك!",
     "prompts":["الأنف!","المسه!"]},
    {"id":"مد_الذراعين","name":"🤸 مد ذراعيك","verify":"arms_out",
     "instruction":"مدَّ ذراعيك للجانبين!","waiting":"مدَّ ذراعيك! 🤸",
     "success":"طيارة! ✅","fail":"افتح ذراعيك للجانبين!",
     "prompts":["للجانبين!","مثل الطيارة!"]},
    {"id":"يدان_فوق","name":"🙌 يداك فوق","verify":"hands_up",
     "instruction":"ارفع يديك للأعلى!","waiting":"يداك فوق! 🙌",
     "success":"نجم! ✅","fail":"ارفع كلتا يديك فوق رأسك!",
     "prompts":["كلتا اليدين!","للأعلى!"]},
    {"id":"قفز","name":"🦘 اقفز","verify":"jump",
     "instruction":"اقفز!","waiting":"اقفز! 🦘",
     "success":"قفزة رائعة! ✅","fail":"ارتفع عن الأرض!",
     "prompts":["اقفز!","للأعلى!","بوينج!"]},
    {"id":"إشارة","name":"👉 أشر","verify":"point",
     "instruction":"أشر بإصبعك!","waiting":"أشر! 👉",
     "success":"ممتاز! ✅","fail":"مدَّ إصبع السبابة!",
     "prompts":["أشر!","الإصبع للأمام!"]},
]

# مفردات الكلمات العربية
WORDS_AR = [
    "تفاحة","كرة","قطة","كلب","فيل","سمكة","جيد","سعيد","اقفز","طائرة ورقية",
    "حب","حليب","العب","أحمر","شمس","شجرة","ماء","نعم","لا","واحد","اثنان",
    "ثلاثة","أربعة","خمسة","ستة","سبعة","ثمانية","تسعة","عشرة","أزرق","أخضر",
    "عصفور","كتاب","كوب","باب","عين","قدم","يد","رأس","أنف","ذراع","رجل",
    "كبير","صغير","ساخن","بارد","فوق","تحت","في","خارج","مساعدة","قف","تعال",
    "اجلس","قف","اركض","امشِ","كل","اشرب","نم","صحِ","افتح","أغلق","أمه","أبي",
    "طفل","صديق","مدرسة","بيت","سيارة","زهرة","مطر","قمر","نجمة","ريح",
]

# عبارات التواصل الاجتماعي
SOCIAL_PHRASES_AR = [
    "مرحبا","شكراً","من فضلك","أريد المزيد","مساعدة","نعم","لا","مع السلامة","آسف",
    "أحبك","كيف حالك","اسمي","أريد","أحتاج","أحسنت","صباح الخير","مساء الخير",
    "أنا بخير","هل يمكنني","صباح النور","أنا جيد","أنا متعب","أنا جائع",
]
...
# ══════════════════════════════════════════════════════════════════
# توليد مجمع المهام — Task Pool Generator
# ══════════════════════════════════════════════════════════════════
def _grid_task_ar(items, prefix, domain, protocol, tokens):
    ...
    return {
        ...
        "name": lbl,
        "instruction": f"أين {lbl}؟ اضغط عليها!",
        "waiting": f"أين {lbl}؟",
        "success": f"صحيح! {lbl}! ✅",
        "fail": f"هذه هي {lbl}!",
        "tablet_mode": "grid",
        "options": opts, "correct": cor,
        "tokens": tokens, "joy": "احتفال",
        "prompts": [f"أين {lbl}؟", "انظر جيداً!", "أنت تستطيع!"],
        "verify": "tablet_click",
    }

def generate_pool_ar():
    pool = []
    # حركي (700)
    for _ in range(700):
        m = random.choice(MOTORS_AR)
        pool.append({**m,
            "id": f"{m['id']}_{random.randint(0,99999)}",
            "base_id": m["id"],
            "domain": "حركي", "protocol": random.choice(["ABA-DTT","ESDM"]),
            "tablet_mode": "motor", "tokens": 2, "joy": "رقص"})
    # ألوان (400)
    for _ in range(400):
        pool.append(_grid_task_ar(COLORS_AR,"لون","معرفي","TEACCH",3))
    # حيوانات (350)
    for _ in range(350):
        pool.append(_grid_task_ar(ANIMALS_AR,"حيوان","معرفي","TEACCH",4))
    # فواكه (300)
    for _ in range(300):
        pool.append(_grid_task_ar(FRUITS_AR,"فاكهة","معرفي","TIE",4))
    # أشكال (250)
    for _ in range(250):
        pool.append(_grid_task_ar(SHAPES_AR,"شكل","معرفي","TEACCH",4))
    # مشاعر (200)
    for _ in range(200):
        pool.append(_grid_task_ar(EMOTIONS_AR,"مشاعر","اجتماعي","ESDM",5))
    # طعام (200)
    for _ in range(200):
        pool.append(_grid_task_ar(FOODS_AR,"طعام","يومي","TIE",4))
    # مركبات (200)
    for _ in range(200):
        pool.append(_grid_task_ar(VEHICLES_AR,"مركبة","معرفي","TEACCH",4))
    # جسم (200)
    for _ in range(200):
        pool.append(_grid_task_ar(BODY_AR,"جسم","لفظي","DTT",4))
    # عد بالأصابع (450)
    for _ in range(450):
        num = random.randint(1, 10)
        pool.append({
            "id": f"عد_{num}_{random.randint(0,99999)}",
            "base_id": f"عد_{num}",
            "domain": "رياضيات", "protocol": "ABA-DTT",
            "name": f"🔢 اعدد {num}",
            "instruction": f"أرني {num} أصابع!",
            "waiting": f"أرفع {num} أصابع 🖐️",
            "success": f"نعم! {num} أصابع! ✅",
            "fail": f"أرني {num} أصابع!",
            "tablet_mode": "number", "target_number": num,
            "verify": "finger_count", "tokens": 5, "joy": "احتفال",
            "prompts": [f"أرني {num}!", "عدّ أصابعك!", "استخدم اليدين!"]})
    # كلمات لفظية (500)
    for _ in range(500):
        word = random.choice(WORDS_AR)
        pool.append({
            "id": f"قل_{word}_{random.randint(0,99999)}",
            "base_id": f"قل_{word}",
            "domain": "لفظي", "protocol": "DTT",
            "name": f"🗣️ قل '{word}'",
            "instruction": f"قل الكلمة: {word}!",
            "waiting": f"قل {word}! 🎤",
            "success": f"سمعتك! {word}! ✅",
            "fail": f"حاول مرة أخرى! قل: {word}!",
            "tablet_mode": "word", "word_text": word,
            "verify": "speech_keyword", "keyword": word,
            "tokens": 4, "joy": "رقص",
            "prompts": [f"قل {word}!", "اضغط الميكروفون!", "بصوت واضح!"]})
    # عبارات اجتماعية (400)
    for _ in range(400):
        phrase = random.choice(SOCIAL_PHRASES_AR)
        pool.append({
            "id": f"عبارة_{phrase.replace(' ','_')}_{random.randint(0,99999)}",
            "base_id": f"عبارة_{phrase.replace(' ','_')}",
            "domain": "اجتماعي", "protocol": "ESDM",
            "name": f"💬 '{phrase}'",
            "instruction": f"قل: {phrase}!",
            "waiting": f"قل {phrase}! 🎤",
            "success": f"رائع! '{phrase}'! ✅",
            "fail": f"حاول: {phrase}!",
            "tablet_mode": "social", "social_text": phrase,
            "verify": "speech_keyword", "keyword": phrase,
            "tokens": 6, "joy": "فرح_كامل",
            "prompts": [f"قل {phrase}!", "أنت تستطيع!", "تكلم!"]})
    # حياة يومية (500)
    for _ in range(500):
        iid,emoji,label,kw,cat = random.choice(PECS_AR)
        pool.append({
            "id": f"يومي_{iid}_{random.randint(0,99999)}",
            "base_id": f"يومي_{iid}",
            "domain": "يومي", "protocol": "TIE",
            "name": f"{emoji} {label}",
            "instruction": f"قل: {kw}! {emoji}",
            "waiting": f"قل! {emoji}",
            "success": f"ممتاز! {label}! ✅",
            "fail": f"حاول مرة أخرى! قل: {kw}!",
            "tablet_mode": "daily", "daily_emoji": emoji, "daily_label": label,
            "verify": "speech_keyword", "keyword": kw,
            "tokens": 5, "joy": "احتفال",
            "prompts": [f"قل {kw}!", "بصوت واضح!", "اضغط الميكروفون!"]})
    random.shuffle(pool)
    log.info(f"✅ تم توليد {len(pool)} مهمة علاجية عربية!")
    return pool

log.info("🎯 توليد المهام العلاجية...")
TASK_POOL_AR = generate_pool_ar()
...
ST = {
    ...
    "score": 0, "tokens": 0, "streak": 0, "consecutive": 0,
    "mastered": 0, "conv_level": 1, "domain": "حركي", "protocol": "ABA-DTT",
    "emotion": "محايد",
...
def LOG(msg, t="info"):
    with _lock:
        e = {"time": datetime.now().strftime("%H:%M:%S"),
             "msg": str(msg)[:120], "type": t, "emo": ST["emotion"]}
        ST["logs"].append(e)
        if len(ST["logs"]) > 600: ST["logs"] = ST["logs"][-600:]
    if t == "success": log.info(f"✅ {msg[:60]}")
    elif t == "fail":  log.warning(f"❌ {msg[:60]}")
    else:              log.info(msg[:60])

def get_next_task():
    recent = set(SESSION_HISTORY)
    candidates = [t for t in TASK_POOL_AR if t.get("base_id","") not in recent]
    if not candidates:
        SESSION_HISTORY.clear(); candidates = TASK_POOL_AR
    clvl = ST["conv_level"]
    if clvl == 1:
        filt = [t for t in candidates if t["domain"] in ["حركي","يومي"]]
    elif clvl == 2:
        filt = [t for t in candidates if t["domain"] in ["حركي","معرفي","رياضيات","يومي","لفظي"]]
    else:
        filt = candidates
...
# ══════════════════════════════════════════════════════════════════
# محاكاة PyBullet — نفس النظام بنصوص عربية
# ══════════════════════════════════════════════════════════════════
_PB_SCRIPT_AR = r"""
...
# لافتات المناطق العلاجية بالعربية
for txt,pos,col in [
    ("ABA حركي",[-3.5,3,0.05],[1,.3,.3,1]),
    ("TEACCH بصري",[3.5,3,0.05],[.3,.7,1,1]),
    ("DTT لفظي",[0,3.8,0.05],[.3,1,.5,1]),
    ("ESDM اجتماعي",[-3.5,-3,0.05],[1,.8,.2,1]),
    ("TIE يومي",[3.5,-3,0.05],[.8,.3,1,1]),
    (f"نجم {child}",[0,0,3.2],[.4,.5,.9,1])]:
...
rx=ry=ph=0.0; lu=0.0
st={"is_speaking":False,"lip":0.0,"sj":False,"blink":False,"ht":0.0}
while True:
    try:
...
class PBWatchdog:
    ...
    def _launch(self):
        try:
            self._proc=subprocess.Popen(
                [sys.executable,"-c",_PB_SCRIPT_AR,self.child],
            ...
            while time.time()<dl:
                try:
                    ...
                    if "PYBULLET_READY" in ln:
                        self._ready=True
                        log.info("✅ محاكاة بيبر تعمل — الروبوت يتحرك ويتكلم!")
                        return True
    ...
    def _supervise(self):
        while self._running:
            ...
            self._ready=False
            log.warning("المحاكاة توقفت — إعادة التشغيل خلال 3 ثواني...")
            time.sleep(3)
    def send(self,**kw):
        now=time.time()
...
# ══════════════════════════════════════════════════════════════════
# محرك الصوت العربي — Arabic Voice Engine (TTS + Arabic voice)
# ══════════════════════════════════════════════════════════════════
class ArabicVoice:
    def __init__(self):
        ...
        try:
            self.e=pyttsx3.init()
            self.e.setProperty("rate", 105)   # أبطأ للأطفال
            self.e.setProperty("volume", 1.0)
            voices = self.e.getProperty("voices")
            # نبحث عن صوت عربي أولاً
            arabic_voice = None
            female_voice  = None
            for v in voices:
                vn = v.name.lower()
                if any(x in vn for x in ["arabic","arab","ar-","hoda","naayf","ar_"]):
                    arabic_voice = v.id; break
                if any(x in vn for x in ["female","zira","samantha","hazel","karen"]):
                    female_voice = v.id
            if arabic_voice:
                self.e.setProperty("voice", arabic_voice)
                log.info(f"✅ صوت عربي: {arabic_voice}")
            elif female_voice:
                self.e.setProperty("voice", female_voice)
                log.info("✅ صوت أنثوي (لا يوجد صوت عربي مثبّت)")
            self.ok=True
            log.info("✅ محرك الصوت جاهز")
        except Exception as ex:
            log.warning(f"محرك الصوت: {ex}")

    def _lip(self, text):
        for word in text.split():
            if not ST["is_speaking"]: break
            d = max(0.06, len(word) / 10.0)
            ST["lip_sync_value"] = min(1.0, 0.5 + random.uniform(0.1,0.4))
            time.sleep(d * 0.55)
            ST["lip_sync_value"] = max(0.05, ST["lip_sync_value"] * 0.3)
            time.sleep(d * 0.45)
    ...
    def say(self, text, wait=True):
        ...
        if not clean: return
        ST["is_speaking"] = True
        ST["session_chat"].append({"role":"بيبر","text":clean,
    ...
    def say_pecs(self, text):
        ...
        def _sp():
            ...
            ST["is_speaking"]=False; ST["pecs_interrupt"]=False
            ST["interrupt_flag"]=False
...
# ══════════════════════════════════════════════════════════════════
# خيط الكاميرا — Camera Thread (MediaPipe)
# ══════════════════════════════════════════════════════════════════
EMOTION_MAP_AR = {
...
class CameraThread(QThread):
    ...
    def _init_mp(self):
        ...
        try:
            ...
            log.info("✅ MediaPipe Pose — كشف الهيكل العظمي")
        except Exception as e: log.warning(f"Pose:{e}")
        try:
            self._hands=mp.solutions.hands.Hands(
                max_num_hands=2,min_detection_confidence=0.60,min_tracking_confidence=0.60)
            log.info("✅ MediaPipe Hands — عدّ الأصابع العشرة")
        except Exception as e: log.warning(f"Hands:{e}")
        try:
            ...
            log.info("✅ MediaPipe FaceMesh — كشف المشاعر ٧ فئات")
        except Exception as e: log.warning(f"FaceMesh:{e}")
            ...
            try:
                ...
                if c.isOpened():
                    ...
                    if ret and f is not None and f.size>0:
                        ...
                        self.cap=c; log.info(f"✅ الكاميرا {idx} تعمل"); return
                    c.release()
            except: pass
        log.warning("لم يتم العثور على كاميرا")

    def _count_fingers(self, lm, label):
        """عدّ الأصابع مع تصحيح جانبية الإبهام"""
    ...
    def _emotion_geo(self, lm, w, h):
        try:
            ...
            lb=lm[107]; le=lm[159]; rb=lm[336]; re=lm[386]
            brow=((lb.y-le.y)+(rb.y-re.y))/2.0
            ...
            conf=self._emo_sm[dom]
            pct={EMOTION_MAP_AR.get(k,k):int(v*100) for k,v in self._emo_sm.items()}
            return EMOTION_MAP_AR.get(dom,"محايد"),conf,pct,mar
        except:
            return "محايد",0.5,{k:0 for k in EMOTION_MAP_AR.values()},0.0

    def _sim_frame(self):
        self._phase+=0.04
        f=np.zeros((480,640,3),dtype=np.uint8); f[:]=(10,12,28)
        cv2.putText(f,"لا توجد كاميرا — وضع المحاكاة",
            (80,240),cv2.FONT_HERSHEY_SIMPLEX,0.8,(100,200,255),2)
        return f
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self._face and fc%3==0:
                try:
                    ...
                    if fres.multi_face_landmarks:
                        ...
                        em,conf,pct,mar=self._emotion_geo(fl,w_,h_)
                ...
                try:
                    ...
                    if hres.multi_hand_landmarks:
                        ...
                        for hidx,hlm in enumerate(hres.multi_hand_landmarks):
                            ...
                            if (hres.multi_handedness and hidx<len(hres.multi_handedness)):
                                label=hres.multi_handedness[hidx].classification[0].label
                            else: label="Right"
                            fc_=self._count_fingers(hlm.landmark,label)
                            total+=fc_
                            wrist=hlm.landmark[0]
                            h_,w_=frame.shape[:2]
                            wx,wy=int(wrist.x*w_),int(wrist.y*h_)
                            cv2.putText(frame,f"{label[0]}:{fc_}",
            ...
            if va and time.time()<=ST.get("verify_timeout",0):
                ...
                elif va in ["arms_out","touch_nose","point"]:
                    ok=ST.get("body_motion",0)>5
                if ok:
            ...
            em_ar=ST["emotion"]; fc_n=ST["finger_count"]
            lip_s="👄كلام" if ST["lip_speaking"] else "·"
            cv2.putText(frame,
                f"{'★'*ST['consecutive']}{'☆'*(3-ST['consecutive'])} | {em_ar} | L{ST['conv_level']} | {lip_s}",
                (8,20),cv2.FONT_HERSHEY_SIMPLEX,0.48,(255,220,0),1)
            cv2.putText(frame,
                f"نقاط:{ST['score']} | انتباه:{ST['attention']}% | أصابع:{fc_n}/10 | شفاه:{ST['lip_motion']:.1f}",
...
# ══════════════════════════════════════════════════════════════════
# مسجل اللمس — Touch Recorder (Whisper + Arabic ASR)
# ══════════════════════════════════════════════════════════════════
class TouchRecorder:
    def __init__(self):
        ...
        try:
            ...
            log.info(f"✅ الميكروفون جاهز (عتبة الطاقة: {self.r.energy_threshold:.0f})")
        except Exception as e: log.warning(f"الميكروفون: {e}")
        if _FW:
            try:
                # نستخدم whisper-base مع دعم العربية
                self.whisper=FW("base",device="cpu",compute_type="int8")
                log.info("✅ Whisper-base جاهز (يدعم العربية والإنجليزية)")
            except Exception as e2:
                log.warning(f"Whisper: {e2}")
    ...
    def _capture(self):
        try:
            with sr.Microphone() as src:
                ...
                ST["mic_level"]=min(1.0,rms/5000.0)
        ...
        if self.whisper:
            try:
                ...
                if text:
                    ...
                    self._check_keyword(text); return text
            except Exception as e: log.warning(f"Whisper: {e}")
        # Tier 2: Google (عربي)
        try:
            text=self.r.recognize_google(audio,language="ar-SA")
            log.info(f"Google AR: '{text}'"); self._check_keyword(text); return text
        except sr.UnknownValueError: pass
        except: pass
        # Tier 3: Google (إنجليزي)
        try:
            text=self.r.recognize_google(audio,language="en-US")
            log.info(f"Google EN: '{text}'"); self._check_keyword(text); return text
        except: pass
        if ST["lip_speaking"]:
            log.info("👄 حركة شفاه رُصدت بدون صوت واضح")
        return ""

    def _check_keyword(self, text):
        """مطابقة ثنائية المستوى مع تحقق الشفاه"""
        kw=ST.get("_current_task_keyword","").strip()
        if not kw or not text: return
        t=text.strip()
        # المستوى الأول: مطابقة دقيقة
        if kw in t or t in kw or kw.lower() in t.lower():
            ST["instant_success"]=True
            ST["last_speech_text"]=t
            log.info(f"🎤 مطابقة دقيقة: '{kw}' ← '{t}'"); return
        # المستوى الثاني: مطابقة جزئية + حركة شفاه
        if (len(kw)>=2 and len(t)>=2 and
                (kw[:2]==t[:2] or kw[-2:]==t[-2:])
                and ST.get("lip_speaking")):
            ST["instant_success"]=True
            ST["last_speech_text"]=t
            log.info(f"🎤 مطابقة جزئية+شفاه: '{kw}' ~ '{t}'")

# ══════════════════════════════════════════════════════════════════
# بطاقات الاختيار — Click Cards
# ══════════════════════════════════════════════════════════════════
class ClickCard(QPushButton):
    def __init__(self, data, idx, parent=None):
        super().__init__(parent)
        self.idx=idx; self._data=data
    ...
    def _set_normal(self):
        ...
        if "color" in d:
            ...
            self.setStyleSheet(f"""QPushButton{{background:{d['color']};
                border-radius:75px;border:5px solid rgba(255,255,255,0.3);
    ...
    def flash_correct(self):
        self.setStyleSheet(self.styleSheet()+"QPushButton{border:8px solid #22c55e!important;}")
    def flash_wrong(self):
        self.setStyleSheet(self.styleSheet()+"QPushButton{border:8px solid #ef4444!important;opacity:.4;}")
    def reset(self): self._set_normal()


# ══════════════════════════════════════════════════════════════════
# أفاتار بيبر العربي — Arabic Pepper Avatar Widget
# ══════════════════════════════════════════════════════════════════
class AvatarWidget(QWidget):
    ...
    def _tick(self):
        self._phase+=0.12 if ST["is_speaking"] else 0.03; self.update()
    ...
    def paintEvent(self, event):
        ...
        # العيون
        ec=QColor(100,180,255)
        if em in ["سعيد","بهجة"] or sj: ec=QColor(100,255,180)
        elif em=="غاضب": ec=QColor(255,80,80)
        elif em=="حزين": ec=QColor(100,120,220)
        ...
        p.drawText(QRect(0,2,200,14),Qt.AlignmentFlag.AlignCenter,
                   f"مستوى المحادثة {ST['conv_level']}")
        p.setPen(QColor(160,140,255)); p.setFont(QFont("Arial",7,QFont.Weight.Bold))
        status=("فرح اجتماعي 🎉" if sj else "يتكلم 🔊" if ST["is_speaking"]
                else "يسجل 🔴" if ST["recording"] else "يستمع 👂" if ST["listening"]
                else "جاهز 💤")
        p.drawText(QRect(0,228,200,20),Qt.AlignmentFlag.AlignCenter,status)
        p.end()

# ══════════════════════════════════════════════════════════════════
# لوحة المشاعر العربية — Arabic Emotion Panel
# ══════════════════════════════════════════════════════════════════
EMO_COLORS_AR={
...
class EmotionPanel(QWidget):
    ...
    def paintEvent(self, event):
        ...
        em=ST["emotion"]
        col=QColor(*EMO_COLORS_AR.get(em,(180,180,180)))
        ...
        emoji=EMO_EMOJIS_AR.get(em,"😐")
        p.drawText(QRect(70,18,60,60),Qt.AlignmentFlag.AlignCenter,f"{emoji}\n{em}")
        pct=ST.get("emotion_pct",{}); y0=90
        for em_ in list(EMO_COLORS_AR.keys()):
            val=pct.get(em_,0); bar=int(val*1.3)
            ec_=QColor(*EMO_COLORS_AR.get(em_,(150,150,150)))
        ...
        if lmw>0:
            lc=QColor(0,255,200) if ST["lip_speaking"] else QColor(100,100,200)
            p.setBrush(QBrush(lc)); p.drawRoundedRect(5,220,lmw,9,4,4)
        p.setPen(QColor(200,200,200)); p.setFont(QFont("Arial",7))
...
# ══════════════════════════════════════════════════════════════════
# شريط PECS العربي — Arabic PECS Bar
# ══════════════════════════════════════════════════════════════════
class PECSWidget(QWidget):
    def __init__(self, parent=None):
        ...
        for iid,emoji,label,kw,cat in PECS_AR:
            ...
            btn.setFont(QFont("Arial",6,QFont.Weight.Bold))
            btn.setFixedSize(70,72)
            ...
            btn.clicked.connect(lambda _,k=kw,lb=label: self._press(lb,k))
            inner_lay.addWidget(btn)
        scroll.setWidget(inner); outer.addWidget(scroll,1)

    def _press(self, label, kw):
        entry={"time":datetime.now().strftime("%H:%M:%S"),
               "name":label,"phrase":kw,"emotion":ST["emotion"]}
        ST["pecs_log"].append(entry)
        if len(ST["pecs_log"])>100: ST["pecs_log"]=ST["pecs_log"][-100:]
        LOG(f"PECS:{label}={kw}","info")
        BRIDGE.sig_pecs.emit(kw)
        if VOICE_REF:
            VOICE_REF.say_pecs(f"{CHILD_NAME} يقول: {kw}")


# ══════════════════════════════════════════════════════════════════
# النافذة الرئيسية العربية — Arabic Main Window (1600×900)
# ══════════════════════════════════════════════════════════════════
def shadow(w, blur=12, color="#000", ox=2, oy=2):
...
class MainWindowAR(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"🤖 بيبر كلينيكال إنفينيتي V6 — {CHILD_NAME}")
    ...
    def _build_ui(self):
        ...
        # ── اليسار: الكاميرا + الأفاتار + إيموشن ─────────────────
        ...
        # شريط الحالة
        sr_=QWidget(); srl=QHBoxLayout(sr_)
        srl.setContentsMargins(6,3,6,3); srl.setSpacing(6)
        ...
        self.lip_bar.setRange(0,100); self.lip_bar.setValue(0)
        self.lip_bar.setTextVisible(False)
        self.lip_bar.setStyleSheet("""QProgressBar{background:#1a1f40;border:none;border-radius:3px;}
            QProgressBar::chunk{background:#00ffc8;border-radius:3px;}""")
        ll.addWidget(self.lip_bar)
        ...
        # منطقة المحادثة
        ch=QLabel("💬 محادثة بيبر | اكتب هنا | Whisper AI + كشف الشفاه")
        ch.setFont(QFont("Arial",8,QFont.Weight.Bold))
        ch.setStyleSheet("color:#a78bfa;background:#0c0f1e;padding:2px;")
        ...
        self.chat_input.setPlaceholderText("اكتب رسالة للمعالج أو اسأل عن خطوات العلاج… (Enter)")
        ...
        av_lbl=QLabel("🤖"); av_lbl.setFont(QFont("Arial",22))
        av_lbl.setStyleSheet("color:#a78bfa;"); hl.addWidget(av_lbl)
        tw_=QWidget(); tl2=QVBoxLayout(tw_); tl2.setSpacing(1)
        self.title_lbl=QLabel("بيبر كلينيكال إنفينيتي V6")
        self.title_lbl.setFont(QFont("Arial",14,QFont.Weight.Bold))
        self.title_lbl.setStyleSheet("color:#a78bfa;")
        shadow(self.title_lbl,blur=8,color="#4f46e5"); tl2.addWidget(self.title_lbl)
        self.child_lbl=QLabel(f"طفل:{CHILD_NAME} | ABA/DTT/TEACCH/ESDM/TIE | 5,200+ مهمة | 50 بطاقة PECS")
        ...
        self.instr_lbl=QLabel("جهاز بيبر يستعد…")
        ...
        self.instr_lbl.setWordWrap(True); il.addWidget(self.instr_lbl)
        rl.addWidget(if_fr)
        ...
        self.lock_ov.setStyleSheet("QLabel{background:rgba(0,0,0,0.52);border-radius:12px;color:#a78bfa;}")
        self.lock_ov.hide()
        ...
        self.fb_lbl=QLabel("في انتظار بيبر…")
        ...
        # زر الميكروفون
        mic_row=QWidget(); mic_lay=QVBoxLayout(mic_row)
        mic_lay.setContentsMargins(0,0,0,0); mic_lay.setSpacing(3)
        ...
        self.rec_status=QLabel("🎤 اضغط الميكروفون — Whisper AI + كشف حركة الشفاه جاهز!")
        ...
        for lbl,attr,col in [
            ("النقاط","stat_score","#a78bfa"),("رموز","stat_tokens","#fbbf24"),
            ("متقن","stat_mastered","#34d399"),("متتالي","stat_streak","#60a5fa"),
            ("تخطي","stat_skipped","#f87171"),("يومي%","stat_daily","#34d399"),
            ("⏱ms","stat_resp","#60a5fa")]:
    ...
    def _connect_bridge(self):
        ...
        BRIDGE.sig_success.connect(self._on_success)
        BRIDGE.sig_fail.connect(self._on_fail)
        BRIDGE.sig_skip.connect(self._on_skip)
        BRIDGE.sig_feedback.connect(lambda m: self.fb_lbl.setText(m))
        ...
        BRIDGE.sig_rec_start.connect(lambda: setattr(ST,"recording",True) or None)
    ...
    def _update_cam(self, qimg):
        ...
        lip="👄كلام" if ST["lip_speaking"] else "·"
        self.finger_lbl.setText(
            f"👦{CHILD_NAME}|أصابع:{fc}|{em}|M{cl}|{lip}|انتباه:{ST['attention']}%")

    def _refresh_stats(self):
        self.stat_score.setText(str(ST["score"]))
        self.stat_tokens.setText(str(ST["tokens"]))
        self.stat_mastered.setText(str(ST["mastered"]))
        self.stat_streak.setText(str(ST["streak"]))
        self.stat_skipped.setText(str(ST["tasks_skipped"]))
        self.stat_daily.setText(f"{ST['skill_يومي']:.0f}%")
        rt=ST["resp_times"]
        if rt: self.stat_resp.setText(str(int(sum(rt[-10:])/len(rt[-10:]))))
        c=ST["consecutive"]
        self.stars_lbl.setText("★"*c+"☆"*(3-c))
        self.stars_lbl.setStyleSheet("color:#fbbf24;" if c else "color:#4b5563;")
        self.mastery_sub.setText(f"{c}/3 | فشل:{ST.get('_fail_count',0)}/{MAX_FAILS}")
        self.reward_lbl.setText(f"⭐{ST['score']}")
        self.clvl_lbl.setText(f"مستوى M{ST['conv_level']}")
        ok=ST["tasks_success"]; fa=ST["tasks_fail"]
        rt2=ST["resp_times"]
        if rt2 and ok+fa>0:
            avg=int(sum(rt2[-5:])/len(rt2[-5:]))
            acc=int(ok/(ok+fa)*100)
            self.resp_lbl.setText(f"⏱ متوسط:{avg}ms | ✅ دقة:{acc}% | ✅{ok} ❌{fa}")
    ...
    def _on_task(self, task:dict):
        if task.get("action")=="click":
            self._handle_click(task.get("idx",0)); return
        self._clear(); self._locked=True; self.lock_ov.hide()
        self.sched_task.setText(f"📋 {task.get('name','مهمة')[:20]}")
        self._build_content(task)

    def _build_content(self, task):
        mode=task.get("tablet_mode","")
        if mode=="motor":
            name_lbl=QLabel(task.get("name",""))
            name_lbl.setFont(QFont("Arial",20,QFont.Weight.Bold))
            name_lbl.setStyleSheet("color:#a78bfa;")
            name_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.content_lay.addWidget(name_lbl)
            instr=QLabel(task.get("instruction",""))
            instr.setFont(QFont("Arial",14,QFont.Weight.Bold))
            instr.setStyleSheet(
                "color:#e0e6ff;background:#1e1b4b;border-radius:12px;border:3px solid #4f46e5;padding:12px;")
            instr.setAlignment(Qt.AlignmentFlag.AlignCenter); instr.setWordWrap(True)
            self.content_lay.addWidget(instr)
            waiting=QLabel(task.get("waiting","افعلها!"))
            waiting.setFont(QFont("Arial",11)); waiting.setStyleSheet("color:#34d399;")
            waiting.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.content_lay.addWidget(waiting)
            done_btn=QPushButton("✅ فعلتها! اضغط هنا")
            done_btn.setFixedHeight(52); done_btn.setFont(QFont("Arial",13,QFont.Weight.Bold))
            ...
            done_btn.clicked.connect(lambda: setattr(ST,"instant_success",True) or None)
        ...
        elif mode in ["word","social","daily"]:
            ...
            word_lbl=QLabel(str(txt))
            word_lbl.setFont(QFont("Arial",28,QFont.Weight.Bold))
            word_lbl.setStyleSheet(
                "color:#a78bfa;background:#1e1b4b;border-radius:12px;"
                "border:3px solid #4f46e5;padding:8px 16px;")
            word_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.content_lay.addWidget(word_lbl)
            hint=QLabel("🎤 اضغط الميكروفون وقل الكلمة!")
            ...
            fb=QLabel(task.get("instruction","افعل المهمة!"))
            fb.setFont(QFont("Arial",14,QFont.Weight.Bold)); fb.setWordWrap(True)
            fb.setStyleSheet("color:#e0e6ff;"); fb.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.content_lay.addWidget(fb)

    def _handle_click(self, idx:int):
        if self._locked: return        # بوابة منع النجاح الزائف
        self._do_lock()
    ...
    def _on_success(self, msg:str):
        ...
        self.fb_icon.setText("🎉"); ST["social_joy_active"]=True
        QTimer.singleShot(3000, lambda: ST.__setitem__("social_joy_active",False))

    def _on_fail(self, msg:str):
        self.fb_lbl.setText(f"❌ {msg}"); self.fb_icon.setText("💪")
        self.fb_lbl.setStyleSheet("color:#ef4444;font-weight:bold;font-size:13px;")

    def _on_skip(self, msg:str):
        self.fb_lbl.setText(f"⏭ {msg}"); self.fb_icon.setText("⏩")
        self.fb_lbl.setStyleSheet("color:#f59e0b;font-weight:bold;font-size:13px;")

    def _on_joy(self, mode:str):
        ST["social_joy_active"]=True
        QTimer.singleShot(4000, lambda: ST.__setitem__("social_joy_active",False))

    def _do_unlock(self):
        self._locked=False; self.lock_ov.hide()
        for c in self._cards: c.setEnabled(True)

    def _do_lock(self):
        self._locked=True; self.lock_ov.show()
    ...
    def _on_mic_press(self):
        ...
        self.mic_btn.setStyleSheet("""QPushButton{background:qlineargradient(x1:0,y1:0,x2:1,y2:0,
            stop:0 #991b1b,stop:1 #dc2626);color:white;border-radius:26px;border:4px solid white;}""")
    ...
    def _on_mic_release(self):
        ...
        self.rec_status.setText("⏳ يعالج الصوت بـ Whisper AI…")
    ...
    def _on_rec_stop(self, text:str):
        ST["recording"]=False; ST["listening"]=False
        if text:
            self.rec_status.setText(f"👂 سمعتك: \"{text}\"")
            self._add_chat("طفل",text)
        else:
            lip="(رُصدت حركة شفاه)" if ST["lip_speaking"] else ""
            self.rec_status.setText(f"❌ لم أسمع شيئاً {lip} — حاول مرة أخرى")
...
لديه اضطراب طيف التوحد المستوى الثاني.
الجلسة الحالية: نقاط={ST['score']}, مشاعر={ST['emotion']}, مستوى={ST['conv_level']}.
أجب بـ 2-3 جمل قصيرة باللغة العربية. كن مشجعاً وسريرياً."""
                resp=mdl.generate_content(ctx+"\nالمعالج/الوالد يقول: "+msg)
                rep=resp.text.strip()[:300]
            except Exception:
                rep=f"مرحباً! {CHILD_NAME} يُبلي بلاءً حسناً اليوم! استمر في التشجيع!"
        else:
            rep=f"مرحباً! أنا بيبر! {CHILD_NAME} يتقدم بشكل رائع اليوم!"
        BRIDGE.sig_chat.emit("بيبر",rep)
        if VOICE_REF: VOICE_REF.say(rep)

    def _add_chat(self, role:str, msg:str):
        t=datetime.now().strftime("%H:%M:%S")
        colour={"بيبر":"#a78bfa","معالج":"#60a5fa","طفل":"#34d399"}.get(role,"#9ca3af")
        icon={"بيبر":"🤖","معالج":"👩","طفل":"👦"}.get(role,"💬")
        self.chat_area.append(
            f'<span style="color:{colour};font-weight:bold">{icon}[{t}]:</span> '
...
# ══════════════════════════════════════════════════════════════════
# محرك العلاج — Arabic Therapy Controller
# ══════════════════════════════════════════════════════════════════
class TherapyController(threading.Thread):
    def __init__(self, voice, pb):
        super().__init__(daemon=True)
        self.v=voice; self.pb=pb
        self._task=None; self._fail=0; self._running=True

    def run(self):
        time.sleep(4.0)
        self.v.say(f"مرحباً {CHILD_NAME}! أنا بيبر روبوت العلاج! يالله نبدأ!", wait=True)
        while self._running:
            try: self._show()
            except Exception as e:
                log.warning(f"خطأ في المحرك: {e}"); time.sleep(1.0)

    def _show(self):
        task=get_next_task()
        self._task=task; self._fail=0
        ST["_fail_count"]=0; ST["instant_success"]=False
        ST["tablet_click_result"]=None; ST["_current_task_keyword"]=""
        kw=task.get("keyword","")
        if kw: ST["_current_task_keyword"]=kw.strip()
        if task.get("verify")=="motor":
            self.pb.send(gaze="child",is_speaking=False)
        else:
            self.pb.send(gaze="tablet",is_speaking=False)
        BRIDGE.sig_reset.emit()
        BRIDGE.sig_instr.emit(task.get("instruction",""))
        BRIDGE.sig_task.emit(task)
        ...
        for attempt in range(MAX_FAILS+2):
            ...
            time.sleep(0.4)
        if ST.get("instant_success"): self._on_success(task)
        else: self._on_skip(task)

    def _run_dtt(self, task, attempt):
        verify=task.get("verify","motor")
        wait_s={"motor":8.0,"tablet_click":12.0,
                 "finger_count":9.0,"speech_keyword":12.0}.get(verify,10.0)
        if verify=="motor":
            ST["verify_action"]=task.get("verify","")
            ST["verify_timeout"]=time.time()+wait_s
        ...
        while time.time()<deadline:
            ...
            if verify=="tablet_click" and ST.get("tablet_click_result"):
                if ST["tablet_click_result"]=="correct":
                    ...
                    self._fail+=1; ST["_fail_count"]=self._fail; return
            if verify=="finger_count":
                if ST["finger_count"]==task.get("target_number",1):
                    ST["instant_success"]=True; return
            if verify=="motor" and ST.get("verify_result"):
                ST["instant_success"]=True; return
            time.sleep(0.08)
        self._fail+=1; ST["_fail_count"]=self._fail
        LOG(f"⏱ انتهى الوقت {attempt+1} — فشل {self._fail}/{MAX_FAILS}","fail")
    ...
    def _on_success(self, task):
        ...
        key={"حركي":"skill_حركي","معرفي":"skill_معرفي","لفظي":"skill_لفظي",
             "رياضيات":"skill_رياضيات","اجتماعي":"skill_اجتماعي","يومي":"skill_يومي"}.get(dom)
        if key: ST[key]=min(100,ST[key]+random.uniform(0.8,2.2))
        if ST["consecutive"]>=3:
            ST["mastered"]+=1; ST["consecutive"]=0
            if ST["score"]%80<pts:
                ST["conv_level"]=min(3,ST["conv_level"]+1)
        msg=task.get("success","أحسنت!")
        LOG(f"✅ {msg} وقت={rt}ms نقاط={ST['score']}","success")
        log_csv(task["id"],dom,task.get("protocol","ABA"),True,
                ST["consecutive"],ST["score"],ST["emotion"],ST["attention"],
                ST["conv_level"],rt)
        BRIDGE.sig_success.emit(msg)
        BRIDGE.sig_joy.emit(task.get("joy","احتفال"))
        self.pb.send(is_speaking=True,sj=True,lip=0.9,gaze="child")
        self.v.say(msg)
        joy=task.get("joy","")
        if joy=="فرح_كامل":
            self.v.say(f"ممتاز يا {CHILD_NAME}! أحسنت! علامة كاملة!")
        elif joy=="رقص":
            self.v.say(f"رائع يا {CHILD_NAME}! استمر!")
        else:
            self.v.say(f"صحيح يا {CHILD_NAME}! عظيم!")
    ...
    def _on_skip(self, task):
        ...
        log_csv(task["id"],task.get("domain","حركي"),task.get("protocol","ABA"),
                False,0,ST["score"],ST["emotion"],ST["attention"],ST["conv_level"],0)
        BRIDGE.sig_skip.emit(msg)
        self.v.say(f"لنجرب مهمة جديدة يا {CHILD_NAME}! أنت تستطيع!")
...
# ══════════════════════════════════════════════════════════════════
# لوحة الوالدين العربية — Arabic Parent Dashboard (Flask)
# ══════════════════════════════════════════════════════════════════
DASHBOARD_AR = r"""
<!DOCTYPE html>
...
:root{--bg:#060918;--bg2:#0c0f1e;--bg3:#1a1f40;
  --purple:#a78bfa;--blue:#3b82f6;--green:#22c55e;
  --red:#ef4444;--yellow:#fbbf24;--text:#e0e6ff;--text2:#9ca3af;}
body{background:var(--bg);color:var(--text);font-family:'Segoe UI',Tahoma,Arial,sans-serif;
  font-size:14px;direction:rtl;}
.topbar{background:linear-gradient(135deg,#1a0a3d,#0a0f28);padding:14px 22px;
  display:flex;align-items:center;gap:14px;border-bottom:2px solid #4f46e5;flex-wrap:wrap;}
.topbar h1{font-size:1.2em;color:var(--purple)}
.badge{background:#1e1b4b;border:1px solid #4f46e5;border-radius:20px;
  padding:4px 12px;font-size:.82em;color:var(--yellow)}
.tabs{display:flex;background:var(--bg2);border-bottom:2px solid var(--bg3);
  overflow-x:auto;flex-wrap:wrap;}
.tab{padding:13px 18px;cursor:pointer;font-size:.85em;font-weight:600;
  color:var(--text2);white-space:nowrap;border-bottom:3px solid transparent;transition:.2s;}
.tab.active,.tab:hover{color:var(--purple);border-bottom-color:var(--purple)}
.content{padding:20px;overflow-y:auto;height:calc(100vh - 115px)}
.card{background:var(--bg2);border-radius:14px;padding:18px;
  border:1px solid var(--bg3);margin-bottom:14px}
.card h3{color:var(--purple);font-size:1em;margin-bottom:12px}
.stat-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));
  gap:10px;margin-bottom:16px}
.stat{background:var(--bg2);border-radius:10px;padding:12px;
...
.skill-bar label{display:flex;justify-content:space-between;
  font-size:.8em;margin-bottom:3px}
.bar-bg{background:var(--bg3);border-radius:5px;height:9px}
.bar-fill{height:9px;border-radius:5px;transition:width 1s}
.diag-item{padding:7px 12px;background:var(--bg3);border-radius:8px;
  margin:4px 0;font-size:.83em;border-right:4px solid #4f46e5}
.treat-item{padding:7px 12px;background:#052918;border-radius:8px;
  margin:4px 0;font-size:.83em;border-right:4px solid var(--green);color:var(--green)}
.log-row{display:flex;justify-content:space-between;padding:5px 0;
  border-bottom:1px solid var(--bg3);font-size:.78em}
.ok{color:var(--green)}.fail{color:var(--red)}
input,select,textarea{width:100%;background:var(--bg3);border:2px solid var(--bg3);
  border-radius:8px;padding:9px 12px;color:var(--text);font-size:.9em;
  margin-bottom:10px;outline:none;direction:rtl}
input:focus,select:focus{border-color:#4f46e5}
.btn{background:#4f46e5;color:#fff;border:none;border-radius:8px;
  padding:10px 20px;font-weight:700;cursor:pointer;width:100%;font-size:.95em}
.btn:hover{background:#4338ca}
.btn-green{background:#059669}.btn-green:hover{background:#047857}
.pecs-grid{display:flex;flex-wrap:wrap;gap:8px}
.pecs-btn{background:#1e1b4b;border:2px solid #4f46e5;border-radius:10px;
  padding:8px 14px;cursor:pointer;font-size:.88em;color:#e0e6ff;transition:.15s}
.pecs-btn:hover{background:#2e2b6e;border-color:#a78bfa}
.child-card{background:var(--bg2);border-radius:10px;padding:12px;
  border:1px solid var(--bg3);margin-bottom:8px;cursor:pointer;
  display:flex;align-items:center;gap:12px}
.child-card:hover{border-color:var(--purple)}
.q-item{margin-bottom:10px;padding:10px;background:var(--bg2);border-radius:10px}
.q-text{font-size:.84em;margin-bottom:6px}
.q-opts{display:flex;gap:6px;flex-wrap:wrap}
.q-opt{flex:1;background:var(--bg3);border:2px solid var(--bg3);border-radius:8px;
  padding:6px;text-align:center;font-size:.75em;cursor:pointer;min-width:60px;transition:.15s}
.q-opt:hover{border-color:#4f46e5}
.q-opt.sel{background:#4f46e5;border-color:#4f46e5;color:#fff}
.score-box{background:#052918;border:2px solid var(--green);border-radius:10px;
  padding:14px;text-align:center;margin-top:10px}
.score-box .sc-num{font-size:2.2em;font-weight:900;color:var(--green)}
.quick-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:10px}
.quick-btn{background:#1e1b4b;border:2px solid #4f46e5;border-radius:10px;
  padding:14px;color:#e0e6ff;cursor:pointer;font-size:.95em;font-weight:700;
  text-align:center;transition:.15s}
.quick-btn:hover{background:#2e2b6e;border-color:#a78bfa}
</style>
</head>
<body>
<div class="topbar">
  ...
  <span class="badge" id="lip_badge">👄 صامت</span>
  <span class="badge">🖐️ <span id="cfingers">0</span>/10</span>
  <span style="margin-right:auto;font-size:.78em;color:var(--text2)">
    <a href="/report_pdf" style="color:var(--purple)">📄 تقرير PDF</a></span>
</div>
<div class="tabs">
  <div class="tab active" onclick="showTab('overview',this)">📊 نظرة عامة</div>
  <div class="tab" onclick="showTab('diagnosis',this)">🧠 التشخيص</div>
  <div class="tab" onclick="showTab('children',this)">👶 الأطفال</div>
  ...
  <div class="tab" onclick="showTab('training',this)">📚 تدريب الوالدين</div>
</div>
<div class="content" id="content">جاري التحميل…</div>
<script>
let st={},qa={},curScreen='isca';
async function fetchSt(){
  try{
    ...
    document.getElementById('lip_badge').textContent='👄 '+(st.lip_speaking?'كلام':'صامت');
    document.getElementById('cfingers').textContent=st.finger_count||0;
  }catch(e){}
}
...
function renderTab(t){
  ...
  if(t==='overview'){
    ...
    const skills=[
      ['حركي (ABA-DTT)','skill_حركي','#a78bfa'],
      ['معرفي (TEACCH)','skill_معرفي','#06b6d4'],
      ['لفظي (DTT)','skill_لفظي','#fbbf24'],
      ['رياضيات','skill_رياضيات','#22c55e'],
      ['اجتماعي (ESDM)','skill_اجتماعي','#f97316'],
      ['يومي (TIE)','skill_يومي','#ec4899'],
    ];
    c.innerHTML=`
      <div class="stat-grid">
        ...
        <div class="stat"><div class="val" style="color:#f97316">${avg||'—'}ms</div><div class="lbl">⏱ متوسط الوقت</div></div>
        <div class="stat"><div class="val" style="color:#34d399">${st.mastered||0}</div><div class="lbl">🏆 مُتقن</div></div>
        <div class="stat"><div class="val" style="color:#f472b6">${st.conv_level||1}</div><div class="lbl">🗣 مستوى المحادثة</div></div>
        <div class="stat"><div class="val" style="color:#60a5fa">${st.attention||0}%</div><div class="lbl">👁 الانتباه</div></div>
      </div>
      <div class="card">
        <h3>🎯 مجالات المهارات</h3>
        ${skills.map(([l,k,col])=>`
          <div class="skill-bar">
            <label><span>${l}</span>
              <span style="color:${col};font-weight:700">${Math.round(st[k]||50)}%</span></label>
            <div class="bar-bg"><div class="bar-fill" style="width:${st[k]||50}%;background:${col}"></div></div>
          </div>`).join('')}
      </div>
      <div class="card">
        <h3>😊 توزيع المشاعر المباشر</h3>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${Object.entries(st.emotion_pct||{}).map(([em,v])=>
            `<span style="background:#1e1b4b;border:1px solid #4f46e5;border-radius:20px;
            padding:3px 10px;font-size:.8em">${em} ${v}%</span>`).join('')}
        </div>
        <div style="margin-top:8px;font-size:.85em;color:var(--text2)">
    ...
    const skills=[
      ['skill_حركي','حركي (ABA-DTT)'],['skill_معرفي','معرفي (TEACCH)'],
      ['skill_لفظي','لفظي (DTT)'],['skill_رياضيات','رياضيات'],
      ['skill_اجتماعي','اجتماعي (ESDM)'],['skill_يومي','يومي (TIE)'],
    ];
    const diags=skills.map(([k,n])=>{
      const v=Math.round(st[k]||50);
      if(v>=65) return `✅ ${n}: ${v}% — أداء جيد`;
      if(v>=45) return `⚠️ ${n}: ${v}% — في طور النمو — يحتاج ممارسة مستمرة`;
      return `🔴 ${n}: ${v}% — منطقة أولوية للتركيز`;
    });
    const treats=[];
    if((st.skill_حركي||50)<50) treats.push('🏃 حركي ABA: 5 تكرارات يومية للتصفيق والتلويح. استخدم التلقين الجسدي والتعزيز الفوري.');
    if((st.skill_لفظي||50)<50) treats.push('🎤 لفظي DTT: تدريب الكلمات 10 دقائق 3 مرات يومياً. احتفل بكل محاولة.');
    if((st.skill_اجتماعي||50)<50) treats.push('👥 اجتماعي ESDM: لعب مشترك 20 دقيقة يومياً. اتبع اهتمام الطفل. أضف اللغة بشكل طبيعي.');
    if((st.skill_يومي||50)<50) treats.push('🌅 يومي TIE: مارس 5 روتينات يومية. نفس التسلسل. جدول بصري. مكافأة صغيرة لكل خطوة.');
    if(!treats.length) treats.push('✨ الطفل يُبلي بلاءً حسناً — حافظ على البرنامج وزد الصعوبة تدريجياً!');
    c.innerHTML=`
      <div class="card">
        <h3>🧠 التشخيص السريري التلقائي — ${st.name||'الطفل'}</h3>
        ${diags.map(d=>`<div class="diag-item">${d}</div>`).join('')}
      </div>
      <div class="card">
        <h3>💊 البروتوكول العلاجي الموصى به</h3>
        ${treats.map(t=>`<div class="treat-item">${t}</div>`).join('')}
      </div>
      <div class="card">
        <h3>📈 ملخص الجلسة</h3>
        <div class="diag-item">الدقة: ${(st.tasks_success||0)+(st.tasks_fail||0)>0?
          Math.round((st.tasks_success||0)/((st.tasks_success||0)+(st.tasks_fail||0))*100):0}%</div>
        <div class="diag-item">مستوى المحادثة: M${st.conv_level||1}</div>
        <div class="diag-item">المشاعر السائدة: ${st.emotion||'محايد'}</div>
        <div class="diag-item">حركة الشفاه: ${st.lip_speaking?'نشطة':'غير نشطة'}</div>
        <div class="diag-item">عدد الأصابع: ${st.finger_count||0}/10</div>
      </div>`;
  }
  else if(t==='children'){
    c.innerHTML=`
      <div class="card">
        <h3>👶 إضافة ملف طفل جديد</h3>
        <input id="nc_name" placeholder="اسم الطفل">
        <input id="nc_age" placeholder="العمر" type="number">
        <select id="nc_level">
          <option value="1">المستوى 1 — خفيف</option>
          <option value="2" selected>المستوى 2 — متوسط</option>
          <option value="3">المستوى 3 — شديد</option>
        </select>
        <input id="nc_notes" placeholder="ملاحظات">
        <button class="btn btn-green" onclick="addChild()">+ إضافة طفل</button>
      </div>
    ...
    const logs=(st.logs||[]).slice().reverse().slice(0,60);
    const pecs=(st.pecs_log||[]).slice().reverse().slice(0,30);
    c.innerHTML=`
      <div class="card">
        <h3>📋 سجل الجلسة (${logs.length} حدث)</h3>
        ${logs.map(l=>`<div class="log-row">
          <span>${l.time} — ${l.msg}</span>
          <span class="${l.type==='success'?'ok':'fail'}">${l.type==='success'?'✅':'ℹ️'}</span>
        </div>`).join('') || '<div style="color:var(--text2);padding:10px">لا أحداث بعد</div>'}
      </div>
      <div class="card">
        <h3>🗣️ سجل PECS (${pecs.length} استخدام)</h3>
        ${pecs.map(p=>`<div class="log-row">
          <span>${p.time} — ${p.name}: "${p.phrase}"</span>
          <span style="color:var(--purple)">${p.emotion||'?'}</span>
        </div>`).join('') || '<div style="color:var(--text2);padding:10px">لا استخدامات بعد</div>'}
      </div>`;
  }
  else if(t==='pecs'){
    const cats={صباح:'☀️ الصباح',نظافة:'🧼 النظافة',طعام:'🍽️ الطعام',
                اجتماعي:'💬 اجتماعي',سلامة:'🛑 السلامة',مشاعر:'😊 المشاعر'};
    c.innerHTML=Object.entries(cats).map(([cat,lbl])=>`
      <div class="card">
        <h3>${lbl}</h3>
        <div class="pecs-grid" id="pecs_${cat}">جاري التحميل…</div>
      </div>`).join('');
    fetch('/api/pecs_items').then(r=>r.json()).then(d=>{
      Object.keys(cats).forEach(cat=>{
        const el=document.getElementById('pecs_'+cat);
        if(!el)return;
        const items=d.items.filter(x=>x.cat===cat);
        el.innerHTML=items.map(x=>`
          <button class="pecs-btn" onclick="sayPECS('${x.kw}','${x.label}')">
            ${x.emoji} ${x.label}
          </button>`).join('');
      });
    });
    ...
    c.innerHTML=`
      <div class="card" style="height:360px;overflow-y:auto" id="chatBox">
        ${chat.map(m=>{
          const col=m.role==='بيبر'?'#a78bfa':m.role==='معالج'?'#60a5fa':'#34d399';
          const icon=m.role==='بيبر'?'🤖':m.role==='معالج'?'👩':'👦';
          return `<div style="margin:6px 0">
            <span style="color:${col};font-weight:bold">${icon}[${m.time}]:</span>
            <span> ${m.text}</span></div>`;
        }).join('') || '<div style="color:var(--text2)">لا محادثة بعد</div>'}
      </div>
      <div style="display:flex;gap:8px;margin-top:10px">
        <input id="chatMsg" placeholder="اكتب رسالة لبيبر أو للمعالج…"
          onkeydown="if(event.key==='Enter')sendChat()">
        <button class="btn" style="width:90px" onclick="sendChat()">إرسال</button>
      </div>`;
    ...
    c.innerHTML=`
      <div class="card">
        <h3>⚡ إجراءات سريعة — التحكم عن بُعد</h3>
        <div class="quick-grid">
          ${[['المهمة التالية','🎯','next'],['استراحة','⏸️','break'],
             ['احتفال','🎉','celebrate'],['إيقاف','⏯️','pause'],
             ['تشجيع','💪','encourage'],['تقرير PDF','📄','report']
          ].map(([l,ic,a])=>`
            <div class="quick-btn" onclick="quickAction('${a}')">
              ${ic}<br><span style="font-size:.8em">${l}</span>
            </div>`).join('')}
        </div>
      </div>
      <div class="card">
        ...
        <div class="stat-grid">
          <div class="stat"><div class="val" style="color:#22c55e">${st.tasks_success||0}</div><div class="lbl">مهام صحيحة</div></div>
          <div class="stat"><div class="val" style="color:#ef4444">${st.tasks_fail||0}</div><div class="lbl">مهام خاطئة</div></div>
    ...
    c.innerHTML=`
      <div class="card">
        <h3>📚 مركز تدريب الوالدين — موارد التوحد العربية</h3>
        <p style="color:var(--text2);font-size:.85em;margin-bottom:12px">
          اضغط على أي موضوع لفتح مقاطع تدريبية على YouTube</p>
        ${[
          ['تحليل السلوك التطبيقي ABA للتوحد','ABA autism therapy Arabic tutorial'],
          ['التدريب الصريح DTT للأطفال','DTT discrete trial training autism'],
          ['TEACCH التعليم المنظم في المنزل','TEACCH structured teaching autism home'],
          ['نظام PECS لتبادل الصور','PECS picture exchange communication autism Arabic'],
          ['مهارات الحياة اليومية للتوحد','daily life skills autism children Arabic'],
          ['عدّ الأصابع وتعليم الأرقام','finger counting numbers autism children'],
          ['التعرف على المشاعر للأطفال','emotion recognition activities autism Arabic'],
          ['التدخل المبكر لاضطراب التوحد','early intervention autism Arabic'],
        ].map(([label,query])=>`
          <div onclick="window.open('https://www.youtube.com/results?search_query=${encodeURIComponent(query)}')"
            style="background:var(--bg3);border-radius:8px;padding:10px 14px;margin:5px 0;
            cursor:pointer;display:flex;align-items:center;gap:10px;border:1px solid var(--bg3)"
...
function renderScreening(c){
  const isca=[
    ...
    'هل يُقيم طفلك تواصلاً بالعيون مع الآخرين؟',
    'هل يُشير طفلك للتشارك في الاهتمام (لا للطلب فحسب)؟',
    'هل يُقلّد طفلك الأفعال والأصوات؟',
    'هل يستخدم طفلك الكلمات أو الإيماءات للتواصل؟',
    'هل يُبدي طفلك اهتماماً بالأطفال الآخرين؟',
    'هل يمارس طفلك اللعب التخيلي؟',
    'هل يفهم طفلك التعليمات البسيطة؟',
    'هل يُبدي طفلك المشاعر بشكل مناسب؟',
    'هل لدى طفلك سلوكيات أو حركات متكررة غير معتادة؟',
  ];
  c.innerHTML=`
    <div class="card">
      <h3>🧪 فحص ISCA للتوحد (10 بنود)</h3>
      <p style="color:var(--text2);font-size:.82em;margin-bottom:10px">
        قيّم: 0=لا قلق، 1=بعض القلق، 2=قلق كبير</p>
      ${isca.map((q,i)=>`
        <div class="q-item">
          <div class="q-text">${i+1}. ${q}</div>
          <div class="q-opts">
            ${[['0','لا قلق'],['1','بعض'],['2','كبير']].map(([v,l])=>`
              <div class="q-opt ${qa['isca_'+i]===v?'sel':''}"
                id="isca_${i}_${v}" onclick="setQ('isca',${i},'${v}',this)">
                ${v} — ${l}
              </div>`).join('')}
          </div>
        </div>`).join('')}
      <button class="btn" onclick="submitISCA()">إرسال نتائج ISCA</button>
      <div id="iscaResult" class="score-box" style="display:none"></div>
    </div>`;
}
...
function submitISCA(){
  let total=0;
  for(let i=0;i<10;i++) total+=(parseInt(qa['isca_'+i])||0);
  ...
  const interp=total>12?'⚠️ قلق مرتفع — يُنصح بإحالة متخصص':
               total>6?'⚠️ قلق متوسط — مراقبة دقيقة مطلوبة':'✅ قلق منخفض — استمر في المراقبة';
  const el=document.getElementById('iscaResult');
  if(el){el.style.display='block';el.innerHTML=`
    <div class="sc-num">${total}/20</div>
    <div style="color:var(--green);font-size:.88em;margin-top:6px">${interp}</div>`;}
}
async function addChild(){
  const name=document.getElementById('nc_name').value.trim();
  if(!name)return;
  await fetch('/api/add_child',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({name,age:document.getElementById('nc_age').value,
...
async function loadChildren(){
  ...
  if(!data.children||!data.children.length){
    el.innerHTML='<div style="color:var(--text2);text-align:center;padding:20px">لا أطفال مضافون بعد</div>';
    return;
  }
  el.innerHTML=data.children.map(ch=>`
    <div class="child-card" onclick="switchChild('${ch.name}')">
      <span style="font-size:2em">👦</span>
      <div>
        <div style="font-weight:700">${ch.name}</div>
        <div style="font-size:.78em;color:var(--text2)">
          العمر:${ch.age} | المستوى:${ch.level} | ${ch.notes||''}</div>
      </div>
    </div>`).join('');
}
...
async function quickAction(action){
  if(action==='report'){window.open('/report_pdf');return;}
...
async function sayPECS(kw,label){
  await fetch('/api/pecs',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({keyword:kw,label})});
}
...
fetchSt(); setInterval(fetchSt,1500);
renderTab('overview');
</script>
</body>
</html>
...
@parent_app.route("/")
def dash(): return render_template_string(DASHBOARD_AR)

@parent_app.route("/api/state")
def api_state():
    return jsonify({k:ST[k] for k in ST
        if isinstance(ST[k],(str,int,float,bool,list,dict,type(None)))})

@parent_app.route("/api/screening",methods=["POST"])
def api_screening():
    d=request.json or {}
    inst=d.get("instrument","isca"); sc=int(d.get("score",0))
    if inst=="isca":
        ST["isca_score"]=sc
        if sc>12: ST["conv_level"]=1
        elif sc>6: ST["conv_level"]=min(2,ST["conv_level"])
    return jsonify({"ok":True,"conv_level":ST["conv_level"]})

@parent_app.route("/api/add_child",methods=["POST"])
def api_add_child():
    d=request.json or {}
    if not hasattr(parent_app,"_children"): parent_app._children=[]
    parent_app._children.append({
        "name":d.get("name","?"),"age":d.get("age","?"),
        "level":d.get("level","2"),"notes":d.get("notes",""),
        "added":datetime.now().strftime("%Y-%m-%d")})
    return jsonify({"ok":True})

@parent_app.route("/api/children")
def api_children():
    return jsonify({"children":getattr(parent_app,"_children",[])})

@parent_app.route("/api/switch_child",methods=["POST"])
def api_switch_child():
    d=request.json or {}; nm=d.get("name","")
    if nm: ST["name"]=nm
    return jsonify({"ok":True})
...
def api_pecs():
    ...
    if kw and VOICE_REF:
        VOICE_REF.say_pecs(f"{ST['name']} يقول: {kw}")
...
def report_pdf():
    if not _PDF: return "reportlab غير متاح — pip install reportlab",501
    buf=BytesIO()
    ...
    story.append(Paragraph(
        f"الطفل: {ST['name']} | التاريخ: {ST['session_date']} | "
        f"النقاط: {ST['score']} | المستوى: M{ST['conv_level']}",styles["Normal"]))
    ...
    story.append(Paragraph("<b>سجل الجلسة (آخر 20 حدث):</b>",styles["Heading3"]))
    for lg in (ST.get("logs",[]))[-20:]:
        story.append(Paragraph(f"[{lg['time']}] {lg['msg'][:80]}",styles["Normal"]))
    story.append(Spacer(1,12))
    story.append(Paragraph("<b>استخدامات PECS:</b>",styles["Heading3"]))
    for p in (ST.get("pecs_log",[]))[-15:]:
        story.append(Paragraph(f"[{p['time']}] {p['name']}: {p['phrase']}",styles["Normal"]))
    doc.build(story); buf.seek(0)
    fname=f"PepperV6_AR_Report_{SAFE_NAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
...
def main():
    ...
    # محرك الصوت العربي
    log.info("🔊 تشغيل محرك الصوت العربي...")
    voice=ArabicVoice(); VOICE_REF=voice

    # محاكاة PyBullet
    log.info("🤖 تشغيل محاكاة بيبر...")
    ...
    # خيط الكاميرا
    log.info("📷 تشغيل خط أنابيب الرؤية الحاسوبية...")
    ...
    # لوحة الوالدين
    log.info("🌐 تشغيل لوحة الوالدين العربية (المنفذ 5007)...")
    threading.Thread(target=run_flask,daemon=True).start()

    print("\n"+"═"*58)
    print(f"  ✅ بيبر كلينيكال إنفينيتي V6 — يعمل!")
    print(f"  👦 الطفل: {CHILD_NAME}")
    print(f"  🖥️  النافذة الرئيسية: 1600×900")
    print(f"  🌐 لوحة الوالدين: http://localhost:5007")
    print(f"  🌐 لوحة الوالدين: http://{LOCAL_IP}:5007")
    print(f"  📊 ملف النتائج: {CSV_FILE}")
    print("═"*58+"\n")

    # فتح لوحة الوالدين تلقائياً
    webbrowser.open("http://localhost:5007")

    def _cleanup():
        log.info("🛑 إيقاف بيبر كلينيكال...")
